...
class ConcurrentArrayListImplTest {

    private List<Integer> threadSafeList = new ArrayListImpl<>();
    ...
    void add_nTimes_nSize() {
        ...
        try {
            ...
            for (int i = 0; i < NUM_WORKERS; i++) {
                executorService.execute(new WorkerThread(ArListAction.ADD));
            }
    ...
    void remove_nTimes_emptyList(){
        try {
            ...
            for (int i = 0; i < NUM_WORKERS; i++) {
                executorService.execute(new WorkerThread(ArListAction.REMOVE));
            }
            System.out.println("main is waiting for others to reach barrier");
            cyclicBarrier.await();
            System.out.println("Finish");
    ...
    void set_uniqVal_allUniq(){
        try {
            ...
            for (int i = 0; i < NUM_WORKERS; i++) {
                executorService.execute(new WorkerThread(ArListAction.SET));
            }
            System.out.println("main is waiting for others to reach barrier");
            cyclicBarrier.await();
            System.out.println("Finish");
    ...
    class WorkerThread implements Runnable {
        ...
        private ArListAction action;

        public WorkerThread(ArListAction action){
            this.action = action;
        }
